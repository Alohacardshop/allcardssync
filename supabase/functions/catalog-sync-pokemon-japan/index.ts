...
async function syncSet(setId: string) {
  ...
  const response = await fetchJsonWithRetry(`${JTCG}/cards?game=pokemon&region=japan&set=${encodeURIComponent(setId)}&limit=1000`, headers);
...
serve(async (req) => {
  ...
  try {
    ...
    const setsResponse = await fetchJsonWithRetry(`${JTCG}/sets?game=pokemon&region=japan`, headers);
    ...
    console.error('Pokemon Japan sync error:', e);
