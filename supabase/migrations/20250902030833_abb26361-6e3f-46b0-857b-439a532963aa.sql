-- Create performance index for inventory aggregation queries (without CONCURRENTLY since we're in a transaction)
CREATE INDEX IF NOT EXISTS idx_intake_items_inventory_sync 
...
CREATE TRIGGER intake_items_shopify_sync
  AFTER INSERT OR UPDATE OR DELETE ON public.intake_items
  FOR EACH ROW
  EXECUTE FUNCTION public.trigger_shopify_inventory_sync();