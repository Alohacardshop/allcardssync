...
-- Add grading_company_in parameter to create_raw_intake_item and year_in parameter
CREATE OR REPLACE FUNCTION public.create_raw_intake_item(
  ...
  grading_company_in text DEFAULT 'PSA'::text
)
...
BEGIN
  ...
  -- Extract image URLs from catalog snapshot (supports imageUrls array, imageUrl string, image_url string, and CGC images.front/rear)
  v_image_urls := NULL;
  IF catalog_snapshot_in IS NOT NULL THEN
    ...
    ELSIF catalog_snapshot_in ? 'images' AND jsonb_typeof(catalog_snapshot_in->'images') = 'object' THEN
      -- CGC format: images.front, images.rear
  ...
  IF v_lot_id IS NULL THEN
    RAISE EXCEPTION 'LOT_ERROR: Failed to create or find an active batch for your session.';
  END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    IF SQLERRM NOT LIKE 'AUTH_REQUIRED:%' 
      ...
      RAISE EXCEPTION 'UNEXPECTED_ERROR: An unexpected error occurred: %. Please try again.', SQLERRM;
    ELSE
      RAISE;
    END IF;
END;
