...
-- Step 1: Update trigger_normalize_tags to remove INSERT-only restriction
-- This allows tag generation on both INSERT and UPDATE when shopify_tags is NULL
...
BEGIN
  ...
  IF NEW.shopify_tags IS NULL THEN
    ...
    IF NEW.grading_company IS NOT NULL AND NEW.grading_company != '' THEN
...
-- Step 2: Backfill existing items that have metadata but NULL shopify_tags
-- This UPDATE will now trigger the updated function to generate tags
UPDATE public.intake_items
SET updated_at = now()
WHERE shopify_tags IS NULL
  AND deleted_at IS NULL
  AND (
    (grading_company IS NOT NULL AND grading_company != '')
    OR (main_category IS NOT NULL AND main_category != '')
    OR (brand_title IS NOT NULL AND brand_title != '')
  );

-- Step 3: Clean up orphaned sync queue entries for deleted items
...
WHERE status = 'queued'
  AND inventory_item_id IN (
    SELECT id FROM public.intake_items WHERE deleted_at IS NOT NULL
  );
