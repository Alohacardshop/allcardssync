-- Remove any existing cron jobs for shopify sync
SELECT cron.unschedule('shopify-auto-sync-every-2min') WHERE EXISTS (
  SELECT 1 FROM cron.job WHERE jobname = 'shopify-auto-sync-every-2min'
);
