-- Fix trigger_shopify_queue_sync to handle NULL updated_by values
...
BEGIN
  ...
  IF TG_OP != 'DELETE' AND (NEW.removed_from_batch_at IS NULL OR COALESCE(NEW.updated_by, '') = 'shopify_webhook') THEN
    RETURN NEW;
  END IF;
