...
-- A5. Safety validation trigger (instead of CHECK constraint)
...
-- Create trigger
DROP TRIGGER IF EXISTS validate_item_lot_owner_trigger ON public.intake_items;
CREATE TRIGGER validate_item_lot_owner_trigger
  BEFORE INSERT OR UPDATE ON public.intake_items
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_item_lot_owner();
