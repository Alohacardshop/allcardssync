-- Update send_intake_items_to_inventory function to use TCGPlayer ID as SKU when available
...
BEGIN
  ...
  FOR i IN 1..array_length(item_ids, 1) LOOP
    BEGIN
      ...
      -- Determine final SKU - prioritize TCGPlayer ID from catalog_snapshot
      final_sku := item_record.sku;
      
      -- Check for TCGPlayer ID in catalog_snapshot and use it as SKU
      IF item_record.catalog_snapshot IS NOT NULL AND 
         item_record.catalog_snapshot ? 'tcgplayer_id' AND
         item_record.catalog_snapshot->>'tcgplayer_id' IS NOT NULL AND
         item_record.catalog_snapshot->>'tcgplayer_id' != '' THEN
        final_sku := item_record.catalog_snapshot->>'tcgplayer_id';
      END IF;
      
      -- Fallback to item ID if no SKU available
      IF final_sku IS NULL OR final_sku = '' THEN
        final_sku := item_ids[i]::text;
      END IF;
      ...
      -- IMPORTANT: Preserve ALL data fields including raw JSON data and image URLs and use TCGPlayer ID as SKU
      ...
      SET 
        ...
        -- Use TCGPlayer ID as SKU when available, otherwise keep existing SKU
        sku = final_sku,
        ...
        image_urls = CASE 
          ...
          WHEN catalog_snapshot ? 'image_urls' AND jsonb_array_length(catalog_snapshot->'image_urls') > 0 THEN catalog_snapshot->'image_urls'
          WHEN catalog_snapshot ? 'image_url' AND catalog_snapshot->>'image_url' != '' THEN jsonb_build_array(catalog_snapshot->>'image_url')
          WHEN catalog_snapshot ? 'photo_url' AND catalog_snapshot->>'photo_url' != '' THEN jsonb_build_array(catalog_snapshot->>'photo_url')
