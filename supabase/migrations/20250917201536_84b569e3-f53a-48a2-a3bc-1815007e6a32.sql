-- Fix admin function to include WHERE clauses for all DELETE operations
...
BEGIN
  ...
  -- Clear based on type with proper WHERE clauses
  CASE clear_type
    WHEN 'all' THEN
      DELETE FROM public.shopify_sync_queue WHERE true;
    WHEN 'completed' THEN
