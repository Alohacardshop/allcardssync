...
-- Drop both overloads and recreate a single definitive version
...
DROP FUNCTION IF EXISTS public.create_raw_intake_item(text, text, integer, text, text, text, text, text, text, numeric, numeric, text, text, text, text, text, text, jsonb, jsonb, text);

-- Recreate single function with all parameters
...
BEGIN
  ...
  v_unique_uid := store_key_in || ':' || v_sku || ':' || shopify_location_gid_in;
  ...
  INSERT INTO public.intake_items (
    ...
    unique_item_uid, image_urls, psa_cert, cgc_cert, created_by
  )
  VALUES (
    ...
    v_unique_uid, to_jsonb(v_image_urls), v_psa_cert, v_cgc_cert, auth.uid()
  )
