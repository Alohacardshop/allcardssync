-- ================================
-- JUSTTCG SYNC SYSTEM MODERNIZATION
-- ================================

-- CLEANUP: Drop old import_jobs table
DROP TABLE IF EXISTS catalog_v2.import_jobs CASCADE;
...
WHERE NOT EXISTS (
  ...
  WHERE s.set_id = c.set_id AND s.game = c.game
);
...
WHERE NOT EXISTS (
  ...
  WHERE c.card_id = v.card_id AND c.provider = v.provider
);
...
-- ================================
-- UPDATE CATALOG_V2 TABLES
-- ================================

-- Update catalog_v2.sets with JustTCG integration
...
-- Update catalog_v2.cards with JustTCG integration  
ALTER TABLE catalog_v2.cards
ADD COLUMN IF NOT EXISTS justtcg_card_id TEXT,
ADD COLUMN IF NOT EXISTS tcgplayer_id BIGINT,
ADD COLUMN IF NOT EXISTS justtcg_metadata JSONB DEFAULT '{}';

-- Update catalog_v2.variants with JustTCG integration
...
-- Set existing sets to 'synced' if they have cards
UPDATE catalog_v2.sets s
SET sync_status = 'synced'
WHERE EXISTS (
  SELECT 1 FROM catalog_v2.cards c 
  WHERE c.set_id = s.set_id AND c.game = s.game
);

-- Update last_synced_at for sets with recent activity
UPDATE catalog_v2.sets 
SET last_synced_at = updated_from_source_at
WHERE updated_from_source_at IS NOT NULL AND sync_status = 'synced';