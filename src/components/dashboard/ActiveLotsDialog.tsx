...
export function ActiveLotsDialog({ open, onOpenChange }: ActiveLotsDialogProps) {
  ...
  const { data: lots, isLoading } = useQuery({
    queryKey: ['active-lots-detail'],
    queryFn: async () => {
      // Fetch active lots
      const { data: lotsData, error: lotsError } = await supabase
        .from('intake_lots')
        .select('id, lot_number, lot_type, created_at, store_key')
        .eq('status', 'active')
      ...
      // Fetch item counts per lot
      const lotIds = lotsData.map(l => l.id);
      const counts: Record<string, number> = {};

      // Count items for each lot
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {format(new Date(lot.created_at), 'MMM d, yyyy h:mm a')} · {lot.item_count} item{lot.item_count !== 1 ? 's' : ''} · {lot.lot_type}
                  </p>
                </div>
                <Button
                  variant="ghost"
