...
export const ShopifySection = React.memo(({ item, detailData, onResync, isResyncing }: ShopifySectionProps) => {
  ...
  // Extract Shopify metadata from snapshot
  const snapshot = item.shopify_snapshot;
  ...
  // Cost from detail data or item
  const cost = detailData?.cost ?? item.cost;
  const vendor = detailData?.vendor ?? item.vendor;
  
  // Last sync info - prefer list item data, fall back to detail data
      ...
      {isNotSynced ? (
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2 p-2 bg-muted/30 rounded-md">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <span className="font-mono text-xs truncate max-w-[180px]">{numericProductId}</span>
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className="h-5 w-5"
            ...
            {variantId && (
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <span className="font-mono text-xs truncate max-w-[180px]">{variantId.split('/').pop()}</span>
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className="h-5 w-5"
            ...
            {inventoryItemId && (
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <span className="font-mono text-xs truncate max-w-[180px]">{inventoryItemId.split('/').pop()}</span>
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className="h-5 w-5"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <span className="text-xs text-muted-foreground block">Shopify Updated</span>
              <p className="text-sm">
                {shopifyUpdatedAt 
              ...
              {shopifyUpdatedAt && (
                <p className="text-xs text-muted-foreground">
                  {formatDistanceToNow(new Date(shopifyUpdatedAt), { addSuffix: true })}
                </p>
              )}
            </div>
          </div>
          ...
          {shopifyTags && shopifyTags.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {shopifyTags.slice(0, 8).map((tag, i) => (
                  <Badge key={i} variant="secondary" className="text-xs">
                    {tag}
                  </Badge>
                ))}
                {shopifyTags.length > 8 && (
                  <Badge variant="outline" className="text-xs">
                    +{shopifyTags.length - 8} more
                  </Badge>
                )}
              </div>
            </div>
